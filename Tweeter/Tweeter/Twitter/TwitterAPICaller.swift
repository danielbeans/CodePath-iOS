...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetString: String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        ...
        TwitterAPICaller.client?.post(updateUrl, parameters: ["status": tweetString], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
                  success()
              }, failure: { (task: URLSessionDataTask?, error: Error) in
                  failure(error)
              })
    ...
    func retweet(tweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let favoriteUrl = "https://api.twitter.com/1.1/statuses/retweet/\(tweetId).json"
        TwitterAPICaller.client?.post(favoriteUrl, parameters: ["id": tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
                  success()
              }, failure: { (task: URLSessionDataTask?, error: Error) in
                  failure(error)
    ...
    func unretweet(tweetId: Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let favoriteUrl = "https://api.twitter.com/1.1/statuses/unretweet/\(tweetId).json"
        TwitterAPICaller.client?.post(favoriteUrl, parameters: ["id": tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
                  success()
              }, failure: { (task: URLSessionDataTask?, error: Error) in
                  failure(error)
